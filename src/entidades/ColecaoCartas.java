...
public class ColecaoCartas {
    ...
    public ColecaoCartas(Jogador jogador, Jogador oponente) {
        ...
        cartas.add(new Criatura("Natureza", 5,
            ...
            if (!oponente.getCampo().getCampo().isEmpty()) {
                Cartas ultimaCarta = oponente.getCampo().getCampo().get(oponente.getCampo().getCampo().size()-1);
        ...
        cartas.add(new Criatura("Anubis", 5,
            ...
            if (!jogador.getCampo().getCemiterio().isEmpty() && jogador.getMana() >= 2) {
                Cartas primeiraCarta = jogador.getCampo().getCemiterio().get(0);
                jogador.getCampo().getCampo().add(primeiraCarta);
                jogador.getCampo().getCemiterio().remove(0);
                ...
                (Runnable) () -> {
                    if (!oponente.getCampo().getCampo().isEmpty()) {
                        ...
                        if (!criaturas.isEmpty()) {
                            Criatura criaturaAleatoria = (Criatura) criaturas.get(new Random().nextInt(criaturas.size()-1));
        ...
        cartas.add(new Encantamento(
                ...
                (Runnable) () -> {
                    if (!jogador.getCampo().getCampo().isEmpty()) {
                        jogador.getCampo().getCemiterio().add(jogador.getCampo().getCampo().remove(0));
                    }
                    if (!oponente.getCampo().getCampo().isEmpty()) {
                        oponente.getCampo().getCemiterio().add(oponente.getCampo().getCampo().remove(0));
                    }
                    jogador.comprarCarta();
        ...
        cartas.add(new Encantamento(
                ...
                (Runnable) () -> {
                    ...
                    if (!jogador.getCampo().getCemiterio().isEmpty()) {
                        jogador.getCampo().getCemiterio().get(0).aplicarEfeito();
