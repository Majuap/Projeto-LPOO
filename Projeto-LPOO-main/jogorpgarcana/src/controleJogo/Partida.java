...
public class Partida {
    ...
    public void turno() throws CartaNaoEncontrada{
        ...
        while (continuarTurno) { // Continua pedindo ações enquanto o jogador não passar a vez
            ...
            int indiceUm = 0;
            while (indiceUm< jogador.getCartasNaMao().size()) {
                System.out.println(indiceUm+"._"+jogador.getCartasNaMao().get(indiceUm));
                indiceUm++;
            }
            System.out.println("Cartas no Campo:\n"+jogador.getCampo().getCampo()+"\n");
            int indiceDois =0;
            while (indiceDois<jogador.getCampo().getCampo().size()) {
                System.out.println(indiceDois+"._"+jogador.getCartasNaMao().get(indiceDois));
                indiceDois++;
            }
            ...
            switch (escolha) {
                case 1: // Invocar, colocar a carta no campo
                    ...
                    if (cartaInvocada instanceof Feitico) {
                        ...
                        jogador.getCartasNaMao().remove(cartaInvocada);
                        encantamentoInvocado.aplicarEfeitoContinuo();
                ...
                case 2: // Atacar o oponente
                    if (!jogador.getCampo().getCriaturasNoCampo().isEmpty()) {
                        System.out.println(jogador.getCampo().getCriaturasNoCampo() + "\n");
                ...
                case 3: // Atacar uma criatura no campo
                    if (!jogador.getCampo().getCriaturasNoCampo().isEmpty() && !oponente.getCampo().getCriaturasNoCampo().isEmpty()) {
                        ...
                        System.out.println(oponente.getCampo().getCriaturasNoCampo() + "\n");
                        System.out.println("Escolha o índice de uma criatura no campo inimigo");
                        int indiceAlvo = scanner.nextInt();
                        Criatura alvo = oponente.getCampo().getCriaturasNoCampo().get(indiceAlvo);
                        alvo.setResistencia(alvo.getResistencia() - criaturaAtacante.getPoder(), oponente, alvo);
                ...
                case 4: // Usar habilidade especial de uma criatura no campo
                    if (!jogador.getCampo().getCriaturasNoCampo().isEmpty()) {
                        ...
                        int indice = scanner.nextInt();
                        Criatura criatura = jogador.getCampo().getCriaturasNoCampo().get(indice);
    ...
    public void turnoAutomatizado() throws CartaNaoEncontrada {
        ...
        while (continuarTurno) { // Enquanto o oponente não passar a vez
            ...
            int escolha = random.nextInt(6) + 1; // Escolhe uma ação entre 1 e 6
            switch (escolha) {
                case 1: // Invocar uma carta
                    if (!oponente.getCartasNaMao().isEmpty()) {
                        ...
                        if (oponente.getMana() >= cartaInvocada.getCustoMana()) {
                            ...
                            if (cartaInvocada instanceof Feitico) {
                                ...
                                encantamento.aplicarEfeitoContinuo();
                ...
                case 3: // Atacar uma criatura no campo do jogador
                ...
                case 4: // Usar habilidade especial de uma criatura
                    if (!oponente.getCampo().getCriaturasNoCampo().isEmpty()) {
                ...
                default:
                    System.out.println("Ação inválida gerada pelo oponente.");
                    break;
            }
        }
    }
